...
public class CSVParser {
     ...
     * 灵活解析日期时间字符串
     * @param dateStr 日期字符串
     * @param fields 完整的字段数组
     * @return 解析后的LocalDateTime
     */
    private LocalDateTime parseDateTime(String dateStr, String[] fields) {
        // 处理空或null值
        if (dateStr == null || dateStr.trim().isEmpty()) {
            return tryExtractDateFromTransactionId(fields);
        }
        ...
        // 尝试从交易号中提取日期
        LocalDateTime transactionIdDate = tryExtractDateFromTransactionId(fields);
        if (transactionIdDate != null) {
            return transactionIdDate;
        }
     ...
     * 尝试从交易号中提取日期
     * @param fields 字段数组
     * @return 提取的日期时间，如果无法提取则返回null
     */
    private LocalDateTime tryExtractDateFromTransactionId(String[] fields) {
        if (fields != null && fields.length > 0 && columnMapping.containsKey("transactionId")) {
            int transactionIdIndex = columnMapping.get("transactionId");
            if (transactionIdIndex < fields.length) {
                String transactionId = fields[transactionIdIndex];
                if (transactionId != null && transactionId.length() >= 8) {
                    try {
                        // 假设交易号前8位是yyyyMMdd格式
                        String dateStr = transactionId.substring(0, 8);
                        int year = Integer.parseInt(dateStr.substring(0, 4));
                        int month = Integer.parseInt(dateStr.substring(4, 6));
                        int day = Integer.parseInt(dateStr.substring(6, 8));

                        System.out.println("从交易号提取日期: " + year + "-" + month + "-" + day);
                        return LocalDateTime.of(year, month, day, 0, 0);
                    } catch (Exception e) {
                        System.err.println("从交易号提取日期失败: " + e.getMessage());
                    }
                }
            }
        }
    ...
    private Transaction parseAlipayTransaction(String[] fields) {
        try {
            ...
            // 在parseAlipayTransaction方法中修改:
            String dateStr = fields[columnMapping.get("date")];
            LocalDateTime transactionDate = parseDateTime(dateStr, fields);
